...
public class FileUploadService {
     ...
     * 资源上传入口
     */
    public CommonResponse uploadResource(String file, ReqUploadFile request) {
        ...
        for (UploadResourceHandler handler : handlers) {
            if (handler.chosen(file, request)) {
                try {
                    ...
                    return CommonResponse.ok(JsonUtil.object2String(result));
                } catch (Exception e) {
                    LoggerUtil.error(String.format("上传文件[%s]失败", file), e);
                    return CommonResponse.fail(I18nConstants.COMMON_INTERNAL_ERROR);
                }
            }
        }
        // 不可能跑到这里来
